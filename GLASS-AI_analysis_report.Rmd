...
editor_options:
  markdown:
    wrap: 80
script_updated: March 18, 2025
---

```{r USER_INPUT, include = FALSE, message = FALSE, warning = FALSE}
#### FILES####
# path to directory where files are stored
## if copying path on Windows '\' needs to be replaced with '/' or '\\'
dir_path <- "path/to/analysis/files"
dir_path <- normalizePath(dir_path,winslash = "/") #convert Windows paths if needed
...
full_grouping_vars <- c("grouping_var1","grouping_var2")
# these are formatted text labels for each grouping variable that will used when making graphs
full_grouping_vars_labels <- c("Grouping Variable 1", "Grouping Variable 2")
...
# make formatted labels for each grouping variable level, if desired.
# use the same structure as above (or just keep full_grouping_order_labels <- full_grouping_order)
#' full_grouping_order_labels <- list(
#' grouping_var1 = c(
#' "Group 1",
#' "Group 2",
#' "Group 3"
...
# define variable used to label parts from individuals (i.e., multiple images from a single individual)
...
sample_info_vars <- full_grouping_vars
sample_info_vars_labels <- full_grouping_vars_labels
...
full_group_comparisons <- list(
  c("group1:groupA","group2:groupA"),
  c("group1:groupA","group3:groupA"),
  c("group2:groupB","group3:groupB")
)
...
```{r custom_functions, include = FALSE, message = FALSE, warning = FALSE}

collapse_ggplotly_geom_bar <- function(ggplotly_obj, reverse = TRUE) {
  #' modify bar graphs produced by ggplotly to collapse after filtering like native plotly graphs
  #'
  #' @description
  #' Sets the 'base' value of each bar trace in a ggplotly object to empty to enable collapsing after data filtering
  #' 
  #' @param ggplotly_obj An interactive plotly graph generated using ggplotly()
  wrong_type_count <- 0
  # set 'base' to empty
  for (i in 1:length(ggplotly_obj$x$data)) {
    if (ggplotly_obj$x$data[[i]]$type == "bar") {
      ggplotly_obj$x$data[[i]]$base <- c()
    } else {
      wrong_type_count <- wrong_type_count + 1
    }
  }
  # invert data within legend groups
  if (reverse) {
    legendgroups <- unlist(sapply(ggplotly_obj$x$data, `[`, "legendgroup"))
    for (i_legendgroup in unique(legendgroups)) {
      legendgroup_indices <- which(legendgroups == i_legendgroup)
      ggplotly_obj$x$data[legendgroup_indices] <- rev(ggplotly_obj$x$data[legendgroup_indices])
    }
  }
  if (wrong_type_count > 0) {
    simpleWarning(sprintf("%d traces were not of 'bar' type.", wrong_type_count))
  }
  return(ggplotly_obj)
}
...
paste_to_text_list <- function(text, sep = " ", punctuation = ",", conjunction = "and", oxford = TRUE) {
  ...
  #' 
  #' @param conjunction A character vector that will be inserted between the last two elements in `text`. (Default = "and")
  #' 
  #' @param oxford If true, inserts `punctuation` before `conjunction` between the last two elements in `text`. (i.e, the Oxford comma) 
  #' 
...
permissive_shapiro <- function(xdf, xformula, p.adj.method = "holm") {
  #' Safely perform Shapiro-Wilk test of normality on data that might violate assumptions.
  #' 
  #' @description
  #' Performs Shapiro-Wilk test of normality for provided dataframe and formula,
  #' while handling errors due to small/big sample size or zero variance.
  #' 
  #' @param xdf A dataframe that includes the columns named in `xformula`.
  #' 
  #' @param xformula A formula defining the response and independent variables to be used in later analyses.
  #' 
  #' @param p.adj.method  Name of the p-value adjustment method to use with rstatix::adjust_pvalue().
  #' 
  #' @returns A formatted output of rstatix::shapiro_test()
  #' 
  #' @noRd
...
validate_groups <- function(xdf, xformula, min_count = 3, max_count = Inf, exclude_zero_var = TRUE) {
  #' Test groups in data frame for statistical assumptions of sample size and variance.
  #' 
  #' @description
  #' Test groups defined within the provided dataframe based on the provided formula
  #' to see if they are too small (< min_count), too large (> max_count), or have zero variance.
  #' 
  #' @param xdf A dataframe that includes the columns named in `xformula`.
  #' 
  #' @param xformula A formula defining the response and independent variables to be used in later analyses.
  #' 
  #' @param min_count The minimum number of samples required in each group.
  #' 
  #' @param max_count The maximum number of samples allowed in each group.
  #' 
  #' @param exclude_zero_var If TRUE, groups with zero variance are flagged.
  #' 
  #' @returns A named list with groups in each flag condition
  #' 
  #' @noRd
  # get grouping variables from formula
  formulaVars <- all.vars(xformula)
...
add_significance_ggplotly <- function(plotly_obj, pairwise_test, omnibus_test, comparison_list) {
  ...
  if (!missing(pairwise_test) && ("rstatix_test" %in% class(pairwise_test))) {
...
run_stats_summary <- function(xdf, xformula) {
  ...
  # test for homogeneity of variance
      ...
      dimnames(homo_var_test) <- list(c("Between Groups", "Within Groups", "Total"), 
      ...
      homo_var_test <- homo_var_test %>% relocate(comparison, df)
      homo_var_violation <- homo_var_test$`p-value`[[1]] < 0.05
      if (is.na(homo_var_violation)){
      homo_var_test <- paste(
  ...
  norm_violation <- NULL
  norm_test <- permissive_shapiro(xdf, xformula)
  if (length(na.omit(norm_test$p.adj)) == 0) { 
    norm_test <- paste("Could not compute Shapiro's test for normality due to an error.
                           Presuming that data is not normally distributed.\n")
    norm_violation <- TRUE
  } else {
...
run_omnibus_test <- function(xdf, xformula, useParametric = "auto", useWelch = "auto", stats_summary = NULL) {
  ...
  if (class(useParametric) == "numeric"){
    if (useParametric > 1 || useParametric < 0) {
      ...
      norm_violation_count <- sum(na.omit(stats_summary[[5]]$`p.adj`) < 0.05)
...
run_posthoc_test <- function(omnibus, comparison_list, useParametric = "auto", fwerCorrectionMethod = "holm") {
  if ("omnibus_test" %in% class(omnibus)) {
    ...
    for (comparison in comparison_list) {
      if ((comparison[[1]] %in% xdf[[tvar]] && comparison[[2]] %in% xdf[[tvar]])) {
...
# read in slide grouping information file
...
} else if ("he_image_id" %in% colnames(Slide_Info)) {
  Slide_Info <- Slide_Info %>% mutate(image_id = he_image_id)
}else if ("hne_image_id" %in% colnames(Slide_Info)) {
  Slide_Info <- Slide_Info %>% mutate(image_id = hne_image_id)
}else if ("slide_id" %in% colnames(Slide_Info)) {
  Slide_Info <- Slide_Info %>% mutate(image_id = slide_id)
} else { Slide_Info <- Slide_Info %>% mutate(image_id = !!as.name(subindividual_var))}
...
if (!("image_id" %in% colnames(Slide_Areas))){
  if ("h_e_image_id" %in% colnames(Slide_Areas)) {
    Slide_Areas <- Slide_Areas %>% mutate(image_id = h_e_image_id)
  } else if ("he_image_id" %in% colnames(Slide_Areas)) {
    Slide_Areas <- Slide_Areas %>% mutate(image_id = he_image_id)
  }else if ("hne_image_id" %in% colnames(Slide_Areas)) {
    Slide_Areas <- Slide_Areas %>% mutate(image_id = hne_image_id)
  }else if ("slide_id" %in% colnames(Slide_Areas)) {
    Slide_Areas <- Slide_Areas %>% mutate(image_id = slide_id)
  }else { Slide_Areas <- Slide_Areas %>% mutate(image_id = !!as.name(subindividual_var))}
}
...
```{r data_formatting, include = FALSE, message=FALSE, warning=FALSE ,collapse = TRUE}
if (redundant_fill) {
...
#this makes a formatted label for the full grouping variable from each grouping variables
full_grouping_label <- paste(full_grouping_vars_labels, collapse = ":")
#this makes a full grouping label for each tumor grade
full_grouping_grade_label <- paste(full_grouping_label, "Tumor Grade", sep = ":")
full_group_list <- expand_grid(!!!full_grouping_order) %>% 
  unite("full_group", sep = ":") %>%
  filter(full_group %in% Slide_Info$full_group)

## Pre-processing of Slide_Areas
Slide_Areas$image_id <- as.character(Slide_Areas$image_id)

# merge Slide Info into Slide Areas
Slide_Areas <- Slide_Areas %>% left_join(Slide_Info, by = "image_id")

# expand tumor grade abbreviations
...
if (sum(str_count(colnames(Tumor_Areas),"grade5")) < 2 || max(Tumor_Areas$grade5_area)==0) { 
  Tumor_Areas$grade5_percentage = 0
  Tumor_Areas$grade5_area = 0
  
  count_tumor_grades_analyzed <- 4
  
  tumor_grade_labels <- c("Grade 4", "Grade 3", "Grade 2", "Grade 1")
  tumor_grade_colors <- c("red", "yellow", "blue", "green")
  names(tumor_grade_colors) <- c("4","3","2","1")

} else {
  tumor_grade_x_limits <- c(tumor_grade_x_limits,"5")
  count_tumor_grades_analyzed <- 5
  Tumor_Areas <- Tumor_Areas %>% 
    mutate(grade5_area = ifelse(is.na(grade5_area), 0, grade5_area),
...
comparison_count <- 0
for (i in 1:count_tumor_grades_analyzed) {
...
# make lists of named vectors for formatting plotly legends
...
full_group_labels <- expand_grid(!!!full_grouping_order_labels) %>%
  unite("label", sep = ":")
# remove full_group_labels that are not present in the data
keep_full_groups <- expand_grid(!!!full_grouping_order) %>% 
  unite("full_group", sep = ":") %>% 
  pull(full_group) %in% Slide_Info$full_group
full_group_labels <- full_group_labels[keep_full_groups,]
...
```{r ihc_processing, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE}
...
```{r, message = FALSE, warning = FALSE,}
kable(as.data.frame(matrix(unlist(full_group_comparisons),
                           ncol = 2, 
                           byrow = TRUE)
                    ) %>% 
        setNames(c("Group 1", "Group2")), 
      caption = "Table 2: Comparisons to be made")
```
...
```{r tumor_size_filtering, echo = FALSE, message = FALSE, warning= FALSE}
if (tumor_size_threshold > 0) {
  filtered_tumors <- Tumor_Areas %>% filter(total_tumor_area < tumor_size_threshold)
  ...
  #update slide areas to reflect tumors that were filtered out based on size
...
```{r remove_unmapped_he_slides, echo = TRUE, class.output="bg-success"}
# remove slides that don't have grouping variable in the provided groups
Slide_Areas_unmapped <- Slide_Areas %>% filter(!(full_group %in% full_group_list$full_group))
...
unmapped_slide_tumor_count <- Tumor_Areas %>% filter(image_id %in% Slide_Areas_unmapped$image_id) %>% nrow()
Tumor_Areas <- Tumor_Areas %>% filter(!(image_id %in% Slide_Areas_unmapped$image_id))
slide_number_to_id <- setdiff(slide_number_to_id, Slide_Areas_unmapped$image_id)

#alert user if there are unmapped H&E slides
unmapped_he_slides_present = FALSE
if (length(Slide_Areas_unmapped$image_id) > 0 && length(Slide_Areas$image_id) > 0) {
  unmapped_he_slides_present = TRUE
} else if (length(Slide_Areas_unmapped$image_id) > 0 && length(Slide_Areas$image_id) == 0) {
  unmapped_he_slides_present = TRUE
} else {
  print("All slides successfully grouped based on selected grouping variable")
}
```

```{r eval = unmapped_he_slides_present, class.output="bg-warning"}
if (unmapped_he_slides_present) {
  if (length(Slide_Areas_unmapped$image_id) > 0 && length(Slide_Areas$image_id) > 0) {
    cat(paste(c("Found", length(Slide_Areas_unmapped$image_id), "slides containing",
    unmapped_slide_tumor_count, "tumors that were unable to be grouped based on the selected grouping variable.
        \nThese slides and their tumors will be excluded from analysis."
    ), collapse = " "))
    kable(select(Slide_Areas_unmapped, "image_id", individual_var, full_group, "number_of_tumors"), caption = "Ungrouped Slides")
  } else {
...
```{r remove_unmapped_ihc_slides, class.output="bg-success"}
ihc_stained_slides_missing = FALSE
if (do_ihc_analysis) {
  for (stain in ihc_stain_list) {
    ...
    if (length(missing_stained_slides) == 0 && length(extra_stained_slides) == 0) {
      ihc_stained_slides_missing = ihc_stained_slides_missing || FALSE
    } else {ihc_stained_slides_missing = TRUE}
  }
  # output success message if all slides mapped
  if (ihc_stained_slides_missing == FALSE){
    cat(paste("Found all expected IHC stained slides."))
  }
}
```

```{r eval = ihc_stained_slides_missing, class.output="bg-warning"}
    if (ihc_stained_slides_missing) {
      ihc_stained_slides_missing = TRUE
      for (stain in ihc_stain_list) {
        expected_stained_slides <- unlist(unique(na.omit(select(Slide_Info, paste(tolower(stain), "image_id", sep = "_")))))
        found_stained_slides <- unlist(unique(na.omit(select(Slide_Areas, paste(stain, "image_id", sep = "_")))))
        missing_stained_slides <- setdiff(expected_stained_slides, found_stained_slides)
        ...
        cat(paste(c("Found", length(missing_stained_slides), stain, 
                    "stained slides missing:\n", missing_stained_slides,
                    "\nFound", length(extra_stained_slides), stain,
                    "stained slides without a matching H&E slide defined.\n",
...
```{r filter_out_excluded_slides, class.output="bg-success"}
...
if ("exclude_reason" %in% colnames(Slide_Info)) {
  ...
  excluded_he_slides_present = FALSE
  if (excluded_count > 0) {
    excluded_he_slides_present = TRUE
  } else {
...
```
```{r eval = excluded_he_slides_present, class.output="bg-warning"}
if (excluded_he_slides_present){
  cat(paste(c(
  ...
  kable(select(Slide_Areas_excluded, "image_id", individual_var, full_group, "number_of_tumors", "exclude_reason"), caption = "Excluded slides")
}
```
...
ggplotly_subtitle(plot1, hoverinfo = "text") %>%
  layout(
    width = min(150 * individual_count, 800),
    xaxis = list(
      tickangle = -90,
...
# perform statistics
graph_formula <- tumor_proportion_in_individual ~ full_group * tumor_grade
stats_summary <- run_stats_summary(graph_data, graph_formula)
if (!skip_stats) {
  omni_test <- run_omnibus_test(stats_summary, useParametric = parametric_test_individuals)
  posthoc_test <- run_posthoc_test(omni_test, full_group_tumor_grade_comparisons)
...
graph_data <- Slide_Areas %>%
  ...
  filter(area_grade %in% tumor_grade_x_limits) %>%
...
plot1 <- graph_data %>%
  ...
  # rearrange graph so highest grade is on top
  mutate(area_grade = factor(area_grade, levels = as.character(c(count_tumor_grades_analyzed:1))
    )
  ) %>%
  filter(area_grade %in% tumor_grade_x_limits) %>% 
  ggplot(aes(
...
ggplotly_subtitle(plot1, hoverinfo = c("text")) %>%
  layout(
    ...
    autosize = F,
    xaxis = list(tickangle = -90, tickfont = list(size = min(10, 0.8 * min(50 * individual_count, 800) / individual_count))),
    width = min(c(150 * individual_count, 800)),
    margin = plotly_margins
  ) %>%
  format_plotly_trace("bar", width = 0.7) %>%
  format_plotly_legend("full_group", full_group_grade_info_list, "label", verbose = T) %>%
...
for (fg in present_full_group_labels$label) {
  ...
  mutate(area_grade = factor(area_grade,
    ...
    geom_point(
      ...
      aes(
        x = tumor_uid,
        y = as.numeric(levels(tumor_grade))[tumor_grade]/(count_tumor_grades_analyzed+1)
      )
    ) +
    scale_y_continuous(
      name = "Proportion of tumor area",
      sec.axis = sec_axis(
        trans = ~ . * (count_tumor_grades_analyzed+1),
        name = "Tumor Grade"
      )
    ) +
    scale_fill_manual(
...
```{r end_of_he_analysis, include = FALSE}
# DONE WITH H&E ANALYSIS
```

```{r add_ihc_headers, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
# make headings
if (do_ihc_analysis) {
...
```{r initialize_ihc_containers, eval = do_ihc_analysis, include = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
...
```{r ihc_graph_001, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
...
```{r ihc_graph_002, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r plot_stained_cell_counts, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
...
```{r ihc_graph_003, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
  plot_caption <-
...
```{r ihc_graph_004, eval = do_ihc_analysis,  echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r ihc_graph_005, eval = do_ihc_analysis,  echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
  plot_caption <-
...
```{r ihc_graph_006, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r plot_stained_cell_proportions, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # add heading
...
```{r ihc_graph_007, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r ihc_graph_008, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r ihc_graph_009, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r ihc_graph_010, eval = do_ihc_analysis,  echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r ihc_graph_011, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r tumor_only_ihc_analysis, eval = do_ihc_analysis}
if (do_ihc_analysis) {
  cat("#### Tumor-only IHC analysis {.tabset .tabset-fade .tabset-pills} \n")
}
```

```{r tumor_stained_cell_count, eval = do_ihc_analysis}
if (do_ihc_analysis) {
  cat("##### Stained cell counts {.tabset .tabset-fade .tabset-pills} \n\n")
}
```

```{r ihc_graph_012, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
...
```{r, echo = FALSE, eval = do_ihc_analysis, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r ihc_graph_013, eval = do_ihc_analysis,  echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
  plot_caption <- "Description of this plot goes here..."
...
```{r ihc_graph_014,eval = do_ihc_analysis,  echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
  plot_caption <- "Description of this plot goes here..."
...
```{r plot_tumor_stained_cell_counts, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # create tabset for each stain and draw plots
  if (ihc_plot_number > 0) {
    plotlistlist2tabs(ihc_plots, plot_captions, tab_names = ihc_stain_labels, tab_level = 6)
...
```{r tumor_stained_cell_proportions, eval = do_ihc_analysis, }
if (do_ihc_analysis) {
  # add heading
  cat("##### Stained cell proportion {.tabset .tabset-fade .tabset-pills} \n")
}
...
```{r ihc_graph_015, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
  plot_caption <-
...
```{r ihc_graph_016, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r ihc_graph_017, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r ihc_graph_018, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
  plot_caption <- "Plot description goes here..."
...
```{r plot_tumor_stained_cell_proportions, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # create tabset for each stain and draw plots
  if (ihc_plot_number > 0) {
    plotlistlist2tabs(ihc_plots, plot_captions, tab_names = ihc_stain_labels, tab_level = 6)
...
```{r tumor_stained_cell_densities, eval = do_ihc_analysis,  echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # add heading
  cat("##### Stained cell density {.tabset .tabset-fade .tabset-pills} \n")
}
...
```{r ihc_graph_019, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
...
```{r ihc_graph_020, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
  plot_caption <- "Average density of positively stained cells by area grade in each group"
...
```{r ihc_graph_021, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
  plot_caption <- "Density of positive nuclei within each grade per slide."
...
```{r ihc_graph_022,eval = do_ihc_analysis,  echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # Describe what the plot shows
  plot_caption <- "Plot description goes here..."
...
```{r peritumor_ihc_analysis, eval = do_ihc_analysis, }
if (do_ihc_analysis) {
  # add headings
...
```{r peritumor_stained_cell_counts, eval = do_ihc_analysis, }
if (do_ihc_analysis) {
  # add headings
  cat("##### Stained cell counts {.tabset .tabset-fade .tabset-pills} \n")
}
...
```{r ihc_graph_023,eval = do_ihc_analysis, results = 'asis'}
if (do_ihc_analysis) {
...
```{r ihc_graph_024, eval = do_ihc_analysis, results = 'asis'}
if (do_ihc_analysis) {
...
```{r ihc_graph_025,eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  ihc_plot_number <- ihc_plot_number + 1
  stain_number <- 0
  ihc_plots_temp <- list()
...
```{r ihc_graph_026,eval = do_ihc_analysis,  echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  ihc_plot_number <- ihc_plot_number + 1
  stain_number <- 0
  ihc_plots_temp <- list()
...
```{r ihc_graph_027,eval = do_ihc_analysis,  echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  ihc_plot_number <- ihc_plot_number + 1
  stain_number <- 0
  ihc_plots_temp <- list()
...
```{r plot_peritumor_stained_cell_counts, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # create tabset for each stain and draw plots
  if (ihc_plot_number > 0) {
    plotlistlist2tabs(ihc_plots, plot_captions, tab_names = ihc_stain_labels, tab_level = 6)
...
```{r ihc_graph_028, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results='asis'}
if (do_ihc_analysis) {
  ihc_plot_number <- ihc_plot_number + 1
  stain_number <- 0
  ihc_plots_temp <- list()
...
```{r plot_multi_stained_cell_counts, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # create tabset for each stain and draw plots
  if (ihc_plot_number > 0) {
    plotlistlist2tabs(ihc_plots, plot_captions, tab_names = ihc_stain_labels, tab_level = 6)
...
```{r multi_ihc_stained_cell_proportions, eval = do_ihc_analysis, }
if (do_ihc_analysis) {
  # add heading
...
```{r ihc_graph_029, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  ihc_plot_number <- ihc_plot_number + 1
  stain_number <- 0
  ihc_plots_temp <- list()
...
```{r plot_multi_ihc_stained_cell_proportions, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # create tabset for each stain and draw plots
  if (ihc_plot_number > 0) {
    plotlistlist2tabs(ihc_plots, plot_captions, tab_names = ihc_stain_labels, tab_level = 6)
...
```{r multi_ihc_stained_cell_densities, eval = do_ihc_analysis, }
if (do_ihc_analysis) {
  # add heading
...
```{r plot_multi_ihcstained_cell_densities, eval = do_ihc_analysis, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
if (do_ihc_analysis) {
  # create tabset for each stain and draw plots
  if (ihc_plot_number > 0) {
    plotlistlist2tabs(ihc_plots, plot_captions, tab_names = ihc_stain_labels, tab_level = 6)
